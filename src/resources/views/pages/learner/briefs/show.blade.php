    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div class="lg:col-span-8 space-y-8">
            <div>
                ...
                <h1 class="text-4xl font-black text-slate-900 leading-tight">{{ $brief->title }}</h1>
            </div>
        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="bg-slate-900 rounded-[2rem] p-8 text-white shadow-xl shadow-slate-200 relative overflow-hidden group">
                ...
                <p class="relative z-10 text-slate-400 text-sm mb-6">Make sure you've covered all the requirements before pushing your work.</p>
                
                <a href="{{ route('learner.briefs.submit', $brief->id) }}" 
                   class="relative z-10 block w-full bg-emerald-500 hover:bg-emerald-600 text-white text-center py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all hover:-translate-y-1 active:scale-95">
            ...
            <div class="bg-white border border-slate-100 rounded-[2rem] p-8 space-y-6">
                <div>
                    <label class="text-[10px] font-black text-slate-300 uppercase tracking-widest block mb-2">Deadline</label>
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-bold text-slate-400 uppercase">{{ \Carbon\Carbon::parse($brief->end_date)->diffForHumans() }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <label class="text-[10px] font-black text-slate-300 uppercase tracking-widest block mb-3">Target Skills</label>
                    <div class="flex flex-wrap gap-2">
                        @foreach($brief->competences as $comp)
                            <span class="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg text-[10px] font-black text-slate-600 uppercase">
                                {{ $comp->code }}
                            </span>
                        @endforeach
                    </div>
                ...
                <div>
                    <label class="text-[10px] font-black text-slate-300 uppercase tracking-widest block mb-3">Instructor</label>
                    <div class="flex items-center gap-3">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($brief->instructor->first_name) }}&background=f1f5f9&color=64748b" class="h-10 w-10 rounded-xl border border-slate-100">
                        <p class="text-sm font-bold text-slate-700">{{ $brief->instructor
            ...
            @if($hasSubmitted)
                <div class="bg-emerald-50 border border-emerald-100 rounded-[2rem] p-6 text-center">
                    <p class="text-emerald-700 font-bold text-xs uppercase tracking-widest">Work Submitted</p>
                    <p class="text-[10px] text-emerald-500 mt-1 italic">You can still update your files until the deadline.</p>
                </div>
            @endif
        </div>
    </div>
