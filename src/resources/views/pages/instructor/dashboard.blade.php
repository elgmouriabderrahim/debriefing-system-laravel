...
<div class="max-w-[1400px] mx-auto">
        ...
        <div class="group bg-white border border-slate-100 rounded-[2.5rem] p-8 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 relative">
            <div class="flex justify-between items-start mb-6">
                ...
                <div class="flex flex-col gap-2">
                    <button onclick="openManageModal({{ $class->id }}, '{{ addslashes($class->name) }}')" class="px-4 py-2 bg-slate-50 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all text-center">
                        Add Learners
                    </button>
                    <button onclick="openSprintModal({{ $class->id }}, '{{ addslashes($class->name) }}', {{ $class->sprints->pluck('id') }})" class="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all text-center">
                        Assign Sprints
                    </button>
                </div>
            </div>
    ...
    <div id="manageModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-6">
    ...
    {{-- ASSIGN SPRINTS MODAL --}}
    <div id="sprintModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-6">
        <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-md" onclick="closeSprintModal()"></div>
        <div class="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl relative p-10 overflow-hidden flex flex-col max-h-[90vh]">
            <div class="mb-6">
                <h3 id="modalSprintTitle" class="text-2xl font-black text-slate-900 mb-1">Classroom Curriculum</h3>
                <p class="text-sm text-slate-500 font-medium">Select sprints to assign to this classroom.</p>
            </div>

            <form action="{{ route('instructor.sprints.assign') }}" method="POST" class="flex flex-col flex-1 overflow-hidden">
                @csrf
                <input type="hidden" name="classroom_id" id="sprintClassId">
                
                <div class="flex-1 overflow-y-auto pr-2 space-y-3 mb-6 custom-scrollbar">
                    @foreach($allSprints as $sprint)
                    <label class="flex items-center p-4 bg-slate-50 rounded-2xl border-2 border-transparent hover:border-indigo-500 transition-all cursor-pointer group">
                        <input type="checkbox" name="sprint_ids[]" value="{{ $sprint->id }}" class="sprint-checkbox w-5 h-5 rounded-lg border-slate-300 text-indigo-600 focus:ring-indigo-500">
                        <div class="ml-4">
                            <p class="text-sm font-black text-slate-900 leading-none group-hover:text-indigo-600">{{ $sprint->name }}</p>
                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Duration: {{ $sprint->duration_days }} Days</p>
                        </div>
                    </label>
                    @endforeach
                </div>

                <button type="submit" class="w-full py-4 bg-indigo-600 text-white font-bold rounded-2xl shadow-xl hover:bg-indigo-700 transition-all">
                    Sync Sprints
                </button>
            </form>
        </div>
...
<script>
    ...
    const sprintTitle = document.getElementById('modalSprintTitle');
    const sprintClassIdInput = document.getElementById('sprintClassId');
    ...
    function openSprintModal(id, name, activeIds) {
        sprintTitle.innerText = "Sprints for: " + name;
        sprintClassIdInput.value = id;
